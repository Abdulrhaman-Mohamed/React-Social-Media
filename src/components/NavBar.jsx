        ...
        <MenuList className="p-1">
          {profileMenuItems.map(({ label, icon }, key) => {
            ...
            return (
              <MenuItem
                key={key}
                onClick={key===profileMenuItems.length-1? removeToken:closeMenu}
                className={`flex items-center gap-2 rounded ${
                     "hover:bg-secondary/10 hover:text-secondary focus:bg-secondary/10 active:bg-secondary/10  "
                }`}
              >
                {React.createElement(icon, {
                  className: `h-4 w-4 ${key===profileMenuItems.length-1 ? "text-red-500" : "text-secondary/50"}`,
                  strokeWidth: 2,
                })}
                <Typography
                  as="span"
                  variant="small"
                  className={`font-normal ${key===profileMenuItems.length-1 ? "text-red-500" : ""}`}
                >
                  {label}
                </Typography>
              </MenuItem>
        ...
        {navListItems.map(({ label, icon }, key) => (
          ...
          >
            <MenuItem  className="flex items-center gap-2 lg:rounded-full text-secondary hover:bg-secondary/10">
              {React.createElement(icon, { className: "h-[18px] w-[18px] text-secondary" })}{" "}
            </MenuItem>
          </Typography>
          </Link>
    ...
    return (
      <>
        ...
        <div className="relative mx-auto flex justify-around items-center text-blue-gray-900">
          <div>
          <IconButton onClick={openDrawer} className=' lg:hidden' variant="text">
